...
class Cell():
    def __init__(self, *args):
        ...
        self.point = args[1]
        delta = args[2]
        self.corners = [Point3D(self.point.x - delta.x, self.point.y - delta.y, 0.0),
                        Point3D(self.point.x + delta.x, self.point.y - delta.y, 0.0),
                        Point3D(self.point.x + delta.x, self.point.y + delta.y, 0.0),
                        Point3D(self.point.x - delta.x, self.point.y + delta.y, 0.0)]
        self.state = 'Free'
        self.f = {'h':0.0, 'g':0.0}
    ...
    def IsInsideObstacle(self, obstacle):
        ...
        for point in self.corners:
            ...
            if (point1.y <= point.y <= point2.y) and (point1.x <= point.x <= point2.x): count += 1
        if count == 0: return False
        hInters = 0
        for point in self.corners + [self.point]:
            for i in range(len(obstacle)):
                point1 = Point3D(obstacle[i]) 
                point2 = Point3D(obstacle[(i + 1) if (i + 1) < len(obstacle) else 0])
                if point2.x == point1.x:
                    if (min([point1.y, point2.y]) < point.y <= max([point1.y, point2.y])) and point2.x >= point.x: hInters += 1
                elif point2.y != point1.y:
                    x = point1.x + ((point2.x - point1.x)/(point2.y - point1.y))*(point.y - point1.y)
                    if (min([point1.x, point2.x]) < x <= max([point1.x, point2.x])) and x >= point.x: hInters += 1
            if (hInters % 2) != 0: return True
        return False
...
class OccupancyGrid():
    def __init__(self, *args):
        ...
        self.__cells = []
        self.__startCell = Cell(0, Point3D(0,0,0), self.__delta)
        self.__goalCell = Cell(0, Point3D(0,0,0), self.__delta)
        self.__solution = []
        self.__visited = []
        self.__possible = []
    ...
    def InitialiseCells(self):
        point = Point3D(self.__minPoint.x + self.__delta.x/2,
                        ...
                        0.0)
        cellID = 0
        while(point.y < self.__maxPoint.y):
            while(point.x <= (self.__maxPoint.x - self.__delta.x/2)):
                self.__cells.append(Cell(cellID, Point3D(point), self.__delta))
                cellID += 1
    ...
    def Draw(self):
        for cell in self.__cells:
            ...
            cell.Draw((0.4,0.4,0.4), self.__delta/2)
        for cell in self.__visited:
            cell.Draw((0.7,0.7,1), self.__delta/4)
        for cell in self.__possible:
            cell.Draw((0.7,1,1), self.__delta/4)
        glLineWidth(5)
        ...
        glEnd()
        self.__startCell.Draw((0,1,0), self.__delta/4)
        self.__goalCell.Draw((1,0,0), self.__delta/4)